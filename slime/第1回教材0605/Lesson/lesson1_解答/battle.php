...
//ユーザーIDを使ってMySQLのユーザーテーブルからデータを取得する。
    ...
    try {
        ...
        $userData['hp'] = $userData['hp'] - ($enemyData['atk'] - $userData['def']);
        $enemyData['hp'] = $enemyData['hp'] - ($userData['atk'] - $enemyData['def']);
        //戦闘計算ここまで//////
        ...
        //カラム名　  => hp = ?
        //検索条件　  => user_id = ?
        $sql = 'UPDATE player SET hp = ? WHERE user_id = ?';
        $stmt = $dbh->prepare($sql);
        //プレイヤーのデータをテーブルへ更新
        $flag = $stmt->execute(array($userData['hp'], $user_id));
        if(!$flag){
            throw new Exception("プレイヤーのupdateに失敗しました。");
        }
        //エネミーのデータをテーブルへ更新
        $flag = $stmt->execute(array($enemyData['hp'], $enemy_id));
        if(!$flag){
            throw new Exception("エネミーのupdateに失敗しました。");
        }
        $result = array();
        $result['player_hp'] = $userData['hp'];
        $result['enemy_hp'] = $enemyData['hp'];
